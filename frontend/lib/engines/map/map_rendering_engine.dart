...
class MapRenderingEngine {
  ...
  Future<void> initialise(
    ...
    final routeByMode = <MapColorMode, Map<String, dynamic>>{
      MapColorMode.speed:
          _geoJsonBuilder.buildRouteFeatureCollectionForMode(segments, MapColorMode.speed.name),
      MapColorMode.elevation: _geoJsonBuilder
          .buildRouteFeatureCollectionForMode(segments, MapColorMode.elevation.name),
      MapColorMode.segment:
          _geoJsonBuilder.buildRouteFeatureCollectionForMode(segments, MapColorMode.segment.name),
    };

    for (final mode in MapColorMode.values) {
      final sourceId = _styleBuilder.routeSourceIdForMode(_routeSourceId, mode);
      final layerId = _styleBuilder.routeLayerIdForMode(mode);
      await controller.addGeoJsonSource(sourceId, routeByMode[mode]!);
      await controller.addLineLayer(
        sourceId,
        layerId,
        _styleBuilder.routeLayerProperties(visible: mode == initialColorMode),
      );
    }
    ...
    final skiTrailLoader = _skiTrailLoader;
    if (skiTrailLoader != null) {
      await skiTrailLoader.loadForSegments(
        controller,
  ...
  Future<void> setColorMode(MapColorMode mode) async {
    ...
    _colorMode = mode;
    await _setOnlyModeVisible(controller, mode);
  }

  Future<void> _setOnlyModeVisible(
    MapLibreMapController controller,
    MapColorMode visibleMode,
  ) async {
    for (final mode in MapColorMode.values) {
      await controller.setLayerProperties(
        _styleBuilder.routeLayerIdForMode(mode),
        _styleBuilder.routeLayerProperties(visible: mode == visibleMode),
      );
    }
  }
