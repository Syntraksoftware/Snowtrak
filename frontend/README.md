...
- Set `MAPTILER_API_KEY` in `.env.local.json`.
- Run with env injection:

```bash
flutter run -t lib/main_dev.dart --dart-define-from-file=.env.local.json
```

- VS Code launch is preconfigured in `.vscode/launch.json` as `Flutter (dev + local env)`.
